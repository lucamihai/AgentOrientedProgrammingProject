...
// ----------------------------------------
      ...
      else if (command == CLONE_AGENT_TO_ALL) {
    	  System.out.println("Am ajuns bine");
    	  
    	  for(Object locationObject : locations.values()) {
    		  Location location = (Location)locationObject;
    		  
    		  CloneAgent(agentName, aid, location, "Clone of " + agentName + " for " + location.getName());
    	  }
...
// ---------------------------------
   ...
   private void CloneAgent(String agentName, AID aid, Location destination, String newAgentName) {
	   MobileAgentDescription mad = new MobileAgentDescription();
       mad.setName(aid);
       mad.setDestination(destination);
       CloneAction ca = new CloneAction();
       ca.setNewName(newAgentName);
       ca.setMobileAgentDescription(mad);
       sendRequest(new Action(aid, ca));
